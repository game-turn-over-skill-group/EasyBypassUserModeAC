...
unsigned char* WIN32K::FileData = 0;
ULONG WIN32K::FileSize = 0;


NTSTATUS WIN32K::Initialize()
{
...
void WIN32K::Deinitialize()
{
    RtlFreeMemory(FileData);
}

int WIN32K::GetExportSsdtIndex(const char* ExportName)
{
    ...
    for (int i = 0; i < 32 && ExportOffset + i < FileSize; i++)
    {
        ...
        {
            SsdtOffset = *(int*)(ExportData + i + 1)-0x1000;
            break;
        }
    }
